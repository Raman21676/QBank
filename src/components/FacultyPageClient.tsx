...
export function FacultyPageClient({ faculty }: { faculty: FacultyManifest }) {
  return (
      ...
      <div className="mx-auto max-w-6xl">
        ...
        >
          ...
          <p className="mt-4 text-slate-500 max-w-xl mx-auto">
            Browse past exam papers and answer sheets for all{" "}
            {faculty.totalSemesters} semesters of TU {faculty.shortName} program.
          </p>
        </motion.div>

        {/* Semester Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {faculty.semesters.map((semester, index) => (
            <motion.div
              key={semester.slug}
              initial={{ opacity: 0, y: 20 }}
